...
class Linear4Bit(torch.nn.Module):
    ...
    def _load_state_dict_pre_hook(
        ...
        if f"{prefix}weight" in state_dict:
            ...
            w_flat = weight.detach().to(torch.float32).reshape(-1)
            w_q4, w_norm = block_quantize_4bit(w_flat, group_size=self._group_size)

            self.weight_q4.copy_(w_q4.to(self.weight_q4.device))
            self.weight_norm.copy_(w_norm.to(self.weight_norm.device))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            ...
            # raise NotImplementedError()
            w_flat = block_dequantize_4bit(self.weight_q4, self.weight_norm)  # float32 1D
            w = w_flat.view(*self._shape)  # (out_features, in_features)
